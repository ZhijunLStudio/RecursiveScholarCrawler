# doi_helper.py - 优化版本，添加超时和更好的错误处理
...
# 请求超时配置
REQUEST_TIMEOUT = 30  # 普通请求的超时时间(秒)
DOWNLOAD_TIMEOUT = 60  # 下载请求的超时时间(秒)
...
def extract_scihub_embed_link(html):
    ...
    try:
        ...
        logger.error(f"Error extracting embed link: {e}")
        return None
...
def download_file(url, title, doi, download_folder, max_retries=3):
    ...
    for attempt in range(max_retries):
        try:
            logger.info(f"Download attempt {attempt+1}/{max_retries} for {title}")
            response = requests.get(url, headers=get_headers(), stream=True, timeout=DOWNLOAD_TIMEOUT)
            ...
            logger.error(f"Download timeout on attempt {attempt+1} for {title}")
...
def get_doi_by_title(title):
    ...
    try:
        logger.info(f"Looking up DOI for: {title}")
        ...
        logger.error(f"CrossRef API timeout for title '{title}'")
        return None
    except Exception as e:
...
def download(identifier, output_directory="downloads"):
    ...
    if isinstance(identifier, str) and identifier.startswith("10."):
        doi = identifier
        title = None
        logger.info(f"Processing download request for DOI: {doi}")
    else:
        title = identifier
        logger.info(f"Processing download request for title: {title}")
    ...
    for mirror in sci_hub_mirrors:
        try:
            ...
            sci_hub_url = mirror + doi
            response = requests.get(sci_hub_url, headers=get_headers(), timeout=REQUEST_TIMEOUT)
            ...
            if response.status_code == 200:
                ...
                # 检查是否找到下载链接
                if not original_url:
                    logger.warning(f"No download link found on {mirror} for {doi}")
                    continue
                ...
                logger.info(f"Found download URL: {original_url}")
            ...
            logger.error(f"Request to {mirror} timed out")
        except requests.exceptions.ConnectionError:
            logger.error(f"Connection error with {mirror}")
        except Exception as e:
            logger.error(f"Error with {mirror}: {e}")
    
    # 如果所有镜像都失败
    return {
        "success": False, 
        "status": "all_mirrors_failed", 
        "doi": doi,
